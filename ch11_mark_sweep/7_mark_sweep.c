...
//-----------------------------------------------------------------------------
//---------------------------------- Tests-------------------------------------
static MunitResult test_field_exists(const MunitParameter params[],
                                     void* data) {
  vm_t* vm = vm_new();
  snek_object_t* lane_courses = new_snek_integer(vm, 20);
  snek_object_t* teej_courses = new_snek_integer(vm, 1);

  // Just access the field to ensure it exists.
...
static MunitResult test_marked_is_false(const MunitParameter params[],
                                        void* data) {
  vm_t* vm = vm_new();
  snek_object_t* lane_courses = new_snek_integer(vm, 20);
  snek_object_t* teej_courses = new_snek_integer(vm, 1);
...
int main() {
  MunitTest tests[] = {{"/test_field_exists", test_field_exists, NULL, NULL,
                        MUNIT_TEST_OPTION_NONE, NULL},
                       {"/test_marked_is_false", test_marked_is_false, NULL,
